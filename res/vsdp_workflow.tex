...
\node[right =of step1,text width=6.6cm,draw,thick,fill=red!20] (step3) {
Compute an \textbf{approximate solution}
{\color{blue} $(\tilde{x},\tilde{y},\tilde{z})$}
of {\color{blue} $\mathcal{P}$} using a conic solver of your choice.
};
\node[below =1.5cm of step3,text width=7.0cm,draw,thick,fill=orange!20] (step4) {
Compute \textbf{rigorous bounds} for the unknown optimal solution
{\color{blue} $(x^{*},y^{*})$}:
\\[1em]
$\begin{aligned}
